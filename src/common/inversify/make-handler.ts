...
export const makeHandler = (
  handlerClass: Abstract<BaseHandler>
): AzureFunction => {
  return async (context: Context, ...args) => {
    ...
    const handlerInstance: BaseHandler = await executionContainer.getAsync(
      handlerClass
    );

    const endTime = process.hrtime(startTime);
    contextLogger.info(
      `Assembled handled. assemblingTime=${getTimeInMilliseconds(endTime)}ms`
    );
...
function provideExecutionLogger(
